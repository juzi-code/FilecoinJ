...
public class TransactionHandler {
    ...
    public byte[] transactionSerialize(Transaction transaction) throws AddressException, IOException {
        ...
        valueBytes = WriteMajorTypeHeaderBuf(valueBytes, FilecoinCnt.MajUnsignedInt);
        ByteString valueByte = new ByteString(valueBytes);

        byte[] gasFeeCapBytes = new BigInteger(transaction.getGasFeeCap()).toByteArray();
        gasFeeCapBytes = WriteMajorTypeHeaderBuf(gasFeeCapBytes, FilecoinCnt.MajUnsignedInt);
        ...
        gasPeremiumBytes = WriteMajorTypeHeaderBuf(gasPeremiumBytes, FilecoinCnt.MajUnsignedInt);
    ...
    private byte[] WriteMajorTypeHeaderBuf(byte[] bytes, int c) {
        if (bytes[0] != 0) {
